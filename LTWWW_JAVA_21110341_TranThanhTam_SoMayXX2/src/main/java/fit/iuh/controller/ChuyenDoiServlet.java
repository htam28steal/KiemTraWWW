...
public class ChuyenDoiServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("Danh sách các loại thuốc".equals(action)) {
			response.sendRedirect(request.getContextPath() + "/ThuocServlet");
		} else {
			if ("Danh sách thuốc".equals(action)) {
				response.sendRedirect(request.getContextPath() + "/DanhSachThuocServlet");
			}else {
				response.sendRedirect(request.getContextPath() + "/ThemThuocServlet");
			}
		}
	}
